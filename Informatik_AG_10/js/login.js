...
async function loadAccounts() {
    try {
        const response = await fetch('/Informatik_AG_10/json/accounts.json');
...
function generateMenuFromConfig() {
    ...
    // Stelle sicher, dass ein Header-Profil-Icon existiert und direkt neben dem Menü steht
    (function ensureHeaderIcon() {
        const navbar = document.querySelector('.navbar');
        if (!navbar) return;
        let headerIcon = document.getElementById('header-profile-icon');
        if (!headerIcon) {
            headerIcon = document.createElement('img');
            headerIcon.id = 'header-profile-icon';
            headerIcon.alt = 'Profil';
            headerIcon.style.display = 'none';
            // Füge das Icon unmittelbar nach dem Menü ein (neben dem Login-Reiter)
            const menuContainer = document.querySelector('.menu');
            if (menuContainer && menuContainer.parentNode) {
                if (menuContainer.nextSibling) menuContainer.parentNode.insertBefore(headerIcon, menuContainer.nextSibling);
                else menuContainer.parentNode.appendChild(headerIcon);
            } else {
                navbar.appendChild(headerIcon);
            }

            // optional: Klick auf das Icon öffnet Login/Account oder geht zur Account-Seite
            headerIcon.addEventListener('click', function() {
                if (currentUser) {
                    // geht zur Account-Seite
                    window.location.href = '/Informatik_AG_10/html/account.html';
                } else {
                    // versuche, Login-Dropdown anzuzeigen
